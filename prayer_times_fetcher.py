...
# Download the timetable for Location
    ...
    def format_timetable(self, location):
        ...
        try:
            ...
            logging.error(f"❌ Failed to load timetable file for {location.upper()}: {e}")
            return False
        ...
        if location == "icci":
            ...
            if not data or "timetable" not in data:
                logging.error(f"❌ Timetable data for {location.upper()} is missing or invalid.")
                return False
            ...
            for month, days in data["timetable"].items():
                ...
                for day, prayers in days.items():
                    if len(prayers) < 6:
                        logging.warning(f"⚠️ Invalid prayer data for {location.upper()} on {month}-{day}: {prayers}")
                        continue
            ...
            logging.error(f"❌ Unsupported location: {location}")
            return False
        ...
        try:
            ...
            logging.info(f"✅ Formatted timetable for {location.upper()} saved successfully.")
        except Exception as e:
            logging.error(f"❌ Failed to save formatted timetable for {location.upper()}: {e}")
            return False
    ...
    def _refresh_timetable(self, location):
        ...
        """
        logging.info(f"🔄 Refreshing timetable for {location.upper()}.")

        # Step 1: Download the timetable
        if not self._download_timetable(location):
            logging.error(f"❌ Failed to download timetable for {location.upper()}.")
            return False

        # Step 2: Format the downloaded timetable
        if not self.format_timetable(location):
            logging.error(f"❌ Failed to format timetable for {location.upper()}.")
            return False

        logging.info(f"✅ Timetable for {location.upper()} successfully refreshed and formatted.")
        return True
    ...
    def _is_file_outdated(self, location):
        ...
        try:
            ...
            logging.warning(f"File {timetable_file} not found. It will be treated as outdated.")
    ...
    def fetch_prayer_times(self, location):
        ...
        if location not in sources:
            logging.error(f"Invalid location provided: {location}. Available locations: {sources}")
